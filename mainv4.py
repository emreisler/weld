...
class MainPage(QWidget):
    ...
    def runCycle(self):
        ...
        #give time to power supply to prevent any I/O error
        time.sleep(0.1)

        self.cycleContinue = True
        if self.CurrDrivenCycleMode :
            #Get time at start of the cycle
            ...
            try:
                ...
                self.currentCycleEnd = False
                self.jumpToStep2 = True
                self.jumpToStep3 = True
                self.jumpToEnd = True

                #Start the cycle loop
                while not self.currentCycleEnd :

                    #Get current time each loop
                    self.currentCycleTime = time.perf_counter()

                    self.cycleTime = self.currentCycleTime - self.time1
                    ...
                    if self.cycleTime > self.cycleTime1 and self.jumpToStep2:
                    ...
                    if self.cycleTime > self.cycleTime1 + self.cycleTime2  and self.jumpToStep3 :
                        ...
                        self.jumpToStep3 = False
                        print("Jumped to step3, voltage3,current3")

                    # Check time every loop and jump to third step parameters if cycletime exceeds set time for 2nd step
                    if self.cycleTime > self.cycleTime1 + self.cycleTime2 + self.cycleTime3 and self.jumpToEnd:
